...
def GLSA(G, S, source, target, max_res, res_min, res_name='res_cost'):
    ...
    while L:
        ...
        for v, e in G.succ[u].items():
            ...
            # check res_min and res_max
            for res in range(0,n_res):
                if "res_max" in G.nodes[v]:
                    if v_label[1][res] > G.nodes[v]["res_max"][res]:
                        add_label = False
                        logger.debug('Max resource of {} is reached'.format(res))
                        break
                if "res_min" in G.nodes[v]:
                    if v_label[1][res] < G.nodes[v]["res_min"][res]:
                        v_label[1][res] = G.nodes[v]["res_min"][res]
